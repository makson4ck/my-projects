...
if line:find('Должность: (.*)') then
    rank = line:gsub('{......}', ''):match('Должность: (.*)')
end
...
imgui.OnFrame(function()
    ...
    if imgui.Button(u8' Обновления и информация', imgui.ImVec2(160, 60)) then
        tab = 4
    end
...
imgui.Text(u8'CODE TOM - Офицер запрашивает дополнительного офицера вооруженного TASER-ом на свою позицию.')
		...
		elseif tab == 7 then
        imgui.CenterText('/asu - Кастом меню выдачи розыска(только для 05)')
...
if imgui.Button(faicons('USER_CHECK') .. u8' Повысить(отыгровка без выдачи ранга)', imgui.ImVec2(-1, 25)) then
    ...
    if imgui.Button(faicons('USER_CHECK') .. u8' Экспертиза наркотиков в машине положительная', imgui.ImVec2(-1, 25)) then 
        ...
        elseif tab == 4 then
            ...
            imgui.CenterText(u8"Разработчик: William_Pauza Brainburg(05)")
            if imgui.BeginTabItem(faicons('handshake_angle')..u8' Обновление и информация') then
                imgui.CenterText(u8"Текущая версия скрипта "..bSettings.update.version)
                imgui.CenterTextOptional("Разработчик William_Pauza 05 Brainburg)", true)
                    ...
                    if imgui.Button(u8"Проверить обновление") then
                        ...
                        if response.status_code == 200 then
                            ...
                            if versionInfo['version'] ~= bSettings.update.version then
                                ...
                                bSettings.update.updateText = "Найдено обновление на версию "..u8(versionInfo['version']).."\n"..u8:decode(versionInfo['description'])
                            else
                                bSettings.update.updateText = "Обновлений не найдено"
                            end
                        else
...
function cmd_spawncar()
	lua_thread.create(function()
		sampSendChat("/rb Внимание! Через 20 секунд будет спавн транспорта.")
		wait(2000)
...
function cmd_incar(id)
    if id == "" then
        sampAddChatMessage("Введи айди игрока:: {FFFFFF}/incar [ID] [1-3].",0x318CE7FF -1)
    else
    lua_thread.create(function ()
